...
from biyesheji.redisaaaaa import redis_


@toolssssss.route('/do', methods=['POST'])
def submit():
    redis_instance = redis_()
    ...
    if task_type == '音乐':
        ccc = redis_instance.music(keyword)
        print(ccc)
        if redis_client.exists('music_data_list'):
            redis_client.delete('music_data_list')

    if task_type == '电影':
        aaa = redis_instance.movie()
        print(aaa)
        if redis_client.exists('movie_data_list'):
            redis_client.delete('movie_data_list')

    if task_type == '新闻':
        bbb = redis_instance.news(keyword)
        print(bbb)
        if redis_client.exists('new_data_list'):
            redis_client.delete('new_data_list')
    ...
    cur = mysql.cursor()
    sql = "INSERT INTO CrawlTasks (type, gjz, start_time, IsTrue) VALUES (%s, %s, %s, %s)"
    cur.execute(sql, (task_type, keyword, start_time.strftime("%H:%M:%S"), 0))
    cur.close()
    mysql.commit()
    ...
    if command:
        ...
        if task_type == '电影':
            ...
            next_temp = 0
            while True:
                temp = redis_client.llen('movie_data_list')
                print("zzzzzzzzzzzzzzzzzzzzzzzzzzzz", temp, next_temp)
                if temp != 0:
                    if temp == next_temp:
                        break
                    else:
                        next_temp = redis_client.llen('movie_data_list')
                time.sleep(5)
            redis_client.set('state', b'True')
            # 记录任务结束时间
            end_time = datetime.now()
            # 更新数据库，设置 end_time，计算时间差，设置 IsTrue 为 1
            cur = mysql.cursor()
            ...
            time_diff = end_time - start_time
            cur.execute(sql,
                        (end_time.strftime("%H:%M:%S"), time_diff, 1, keyword, start_time.strftime("%H:%M:%S")))
            cur.close()
            mysql.commit()
            time.sleep(3)
            redis_client.set('state', 'True')
        ...
        if task_type == '音乐':
            ...
            next_temp = 0
            while True:
                temp = redis_client.llen('music_data_list')
                print("zzzzzzzzzzzzzzzzzzzzzzzzzzzz", temp, next_temp)
                if temp != 0:
                    if temp == next_temp:
                        break
                    else:
                        next_temp = redis_client.llen('music_data_list')
                time.sleep(5)
            redis_client.set('state', b'True')
            # 记录任务结束时间
            end_time = datetime.now()
        ...
        if task_type == '新闻':
            ...
            next_temp = 0
            while True:
                temp = redis_client.llen('new_data_list')
                print("zzzzzzzzzzzzzzzzzzzzzzzzzzzz", temp, next_temp)
                if temp != 0:
                    if temp == next_temp:
                        break
                    else:
                        next_temp = redis_client.llen('new_data_list')
                time.sleep(8)
            redis_client.set('state', b'True')
            # 记录任务结束时间
            end_time = datetime.now()
